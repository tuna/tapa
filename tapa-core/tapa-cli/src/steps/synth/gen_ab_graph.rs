...
/// Floorplan config schema — only the field we consume.
#[derive(Debug, Clone, Deserialize)]
struct FloorplanConfig {
    #[serde(default)]
    cpp_arg_pre_assignments: BTreeMap<String, String>,
}
...
fn read_cpp_arg_pre_assignments(floorplan_config: &Path) -> Result<BTreeMap<String, String>> {
    ...
    let cfg: FloorplanConfig = serde_path_to_error::deserialize(de).map_err(|e| {
        CliError::InvalidArg(format!(
            "`{}` schema error at {}: {}",
            floorplan_config.display(),
            e.path(),
    ...
    Ok(cfg.cpp_arg_pre_assignments)
}
...
mod tests {
    ...
    /// Typed deserialization should reject a non-string value inside
    /// `cpp_arg_pre_assignments` and surface the path via
    /// `serde_path_to_error`.
    #[test]
    fn typed_floorplan_config_rejects_bad_preassignments() {
        let dir = tempfile::tempdir().expect("tempdir");
        let fp_cfg = dir.path().join("fp.json");
        fs::write(&fp_cfg, r#"{"cpp_arg_pre_assignments": {"x": 42}}"#).expect("write");
        let err = emit_ab_graph(dir.path(), &minimal_design(), &fp_cfg).expect_err("must error");
        let msg = format!("{err}");
        assert!(
            msg.contains("cpp_arg_pre_assignments.x"),
