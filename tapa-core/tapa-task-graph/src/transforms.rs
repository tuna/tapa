...
//!   retargeting, cross-slot FIFO update) is implemented; the per-slot
//!   C++ wrapper produced by Python's tree-sitter-heavy `gen_slot_cpp`
//!   is **not** ported, so each slot's `code` field carries the original
//!   top's body with a leading TODO marker.
...
///
/// **Scope note**: the structural transform is faithful, but the
/// per-slot C++ wrapper produced by Python's `gen_slot_cpp` is not
/// ported. Each slot's `code` field is the original top's body with a
/// leading TODO marker so HLS surfaces a clear failure until the
/// wrapper lands.
