"""RTL state holder for Task codegen — sets up FSM module handshake ports."""
...
class TaskRtlState:
    ...
    Creating this object sets up the FSM module with handshake ports for
    upper-level tasks.  The task's ``module`` and ``fsm_module`` attributes
    are lightweight placeholders created by ``Task.__init__``; this class
    performs the codegen-specific initialization on top of them.
    """
