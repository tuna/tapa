...
# ruff: noqa: C901,PLR0912,PLR0914
...
def add_m_axi(task: Task, width_table: dict[str, int], files: dict[str, str]) -> None:
    ...
    for arg_name, mmap in task.mmaps.items():  # noqa: PLR1702
                ...
                for axi_port, direction in axi_ports:
                    ...
                    if axi_port == "ID":
                        ...
                        if id_width != s_axi_id_width and direction == "output":
                            port_arg = (
                                f"{{{s_axi_id_width - id_width}'d0, {signal_name}}}"
                            )
                    portargs.append(
                        make_port_arg(
