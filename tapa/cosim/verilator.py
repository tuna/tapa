...
def _detect_xilinx_ips(rtl_dir: Path) -> list[str]:
    ...
    Scans wrapper modules for instantiations of *_ip submodules (which are
    typically encrypted Xilinx IPs), detects the floating-point operation
    type from the module name, and generates behavioral Verilog replacements
    using DPI-C.
        ...
        for ip_module in ip_insts:
            ...
            if ip_path.exists():
                ...
                if "`pragma protect" not in ip_content:
                    continue
            # Determine FP operation from module name
            dpi_func = _detect_fp_operation(ip_module)
            if dpi_func is None:
                _logger.warning(
                ...
                continue
            # Generate behavioral replacement
            replacement = _generate_fp_ip_replacement(ip_module, dpi_func)
            ip_path.write_text(replacement, encoding="utf-8")
            replacements.append(ip_module)
            _logger.info(
                "Generated behavioral replacement: %s (using %s)",
                ip_module,
                dpi_func,
            )
...
def _detect_fp_operation(module_name: str) -> str | None:
    """Detect the floating-point operation from a module name."""
    lower = module_name.lower()
    for pattern, (func, _width) in _FP_OP_MAP.items():
        if f"_{pattern}_" in lower or f"_{pattern}s_" in lower:
            return func
    return None


def _generate_fp_ip_replacement(module_name: str, dpi_func: str) -> str:
