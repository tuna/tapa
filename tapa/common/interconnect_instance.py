...
class InterconnectInstance(Base):
    ...
    def to_dict(
        ...
        ic_name = self.global_name if interconnect_global_name else self.name
            ...
            for task_name, inst in insts.items():
                for idx, task in enumerate(inst):
                    for arg in task["args"].values():
                        if arg["arg"] == ic_name and arg["cat"].startswith(prefix):
                            return task_name, idx
            msg = f"invalid consumed/produced_by for interconnect {ic_name}"
