...
| `TAPA_STALL_WARN_SECONDS` | `10` | Seconds a task may stay blocked, with no poll it makes succeeding, before software simulation warns and names the channel it is blocked on. The accounting is per scheduler thread, not per channel: any successful poll on that thread restarts the count, so a task stays quiet while its thread keeps getting served on other channels. Read once per process, on the first blocked poll. Set `0` to disable. Suppressed while a kernel instance is running, since waiting on hardware or RTL is not a stall. |
