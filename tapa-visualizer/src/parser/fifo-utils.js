...
/** Groups for fifos like fifo_xx[0], fifo_xx[1]...
...
/** Format fifo group indices: [1,2,3,5] -> "1~3,5"
 * @type {(indexes: number[]) => string} */
export const getIndexRange = indexes => {
  ...
  return groups.map(g => g.length === 1 ? g[0] : `${g[0]}~${g.at(-1)}`).join(",");
};
...
const getSubTaskKey = (by, grouping) => grouping !== "merge" ? `${by[0]}/${by[1]}` : by[0];
...
const getProducedMissingStyle = (node, fifoName) => ({
  sourcePort: getPortKey(node, fifoName), targetPort: fifoName, stroke: altEdgeColor,
});
...
export const addFifo = params => {
  ...
  else if (!fifo.produced_by && fifo.consumed_by) addProducedMissingFifo(params);
  else if (fifo.produced_by && !fifo.consumed_by) addConsumedMissingFifo(params);
  else console.warn(`fifo ${fifoName} without produced_by and consumed_by in ${taskName}:`, params);
};
