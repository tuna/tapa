...
void load_S_matrix(FILE* f_A, int nnz_mmio, int& nnz, vector<int>& cooRowIndex,
                   vector<int>& cooColIndex, vector<double>& cooVal,
                   MM_typecode& matcode) {
...
void read_suitsparse_matrix_FP64(char* filename_A, vector<int>& elePtr,
                                 vector<int>& eleIndex, vector<double>& eleVal,
                                 int& M, int& K, int& nnz,
  ...
  if (mf == CSR) {
    sort_by_fn(nnz, cooRowIndex, cooColIndex, eleVal,
               cmp_by_row_column<double>);
  } else if (mf == CSC) {
    sort_by_fn(nnz, cooRowIndex, cooColIndex, eleVal,
               cmp_by_column_row<double>);
  } else {
    cout << "Unknow format!\n";
    exit(1);
  }
...
template <typename data_t>
void cpu_spmv_CSR(const int M, const int K, const int NNZ, const data_t ALPHA,
                  const vector<int>& CSRRowPtr, const vector<int>& CSRColIndex,
                  const vector<data_t>& CSRVal, const vector<data_t>& vec_X,
                  const data_t BETA, vector<data_t>& vec_Y) {
  ...
  for (int i = 0; i < (NUM_COLUMN + WINDOE_SIZE - 1) / WINDOE_SIZE; ++i) {
    ...
    for (int col = WINDOE_SIZE * i;
      ...
      for (int j = CSCColPtr[col]; j < CSCColPtr[col + 1]; ++j) {
        ...
        tmp_edge_list_pes[p][pos] =
            edge<data_t>(col, CSCRowIndex[j], CSCVal[j]);
      }
    }
...
void edge_list_64bit_fp64(
  ...
  int sparse_A_fpga_chunk_size =
      ((sparse_A_fpga_column_size + 255) / 256) * 256;  // 4 KB
      ...
      for (int j = 0; j < 4; ++j) {
        ...
        if (e.row == -1) {
          ...
          /*
           ...
           x_col = (x_col & 0x3FFF) << (64 + 18); // x_col = (x_col & 0xFFF) <<
           (32 + 20); //x_col = (x_col & 0x3FF) << (32 + 22); ap_uint<96> x_row
...
void edge_list_64bit_fp32(
    const vector<vector<edge<float>>>& edge_list_pes,
    const vector<int>& edge_list_ptr,
    ...
    for (int cc = 0; cc < NUM_CH_SPARSE; ++cc) {
      for (int j = 0; j < 8; ++j) {
        ...
        if (NUM_CH_SPARSE == 24) {
          ...
          sparse_A_fpga_vec[(pix_m24 % 4) * 6 + pix_m24 / 4]
                           [(pe_idx % 192) / 24 + i * 8] = x;
...
void CSC_2_CSR(int M, int K, int NNZ, const vector<int>& csc_col_Ptr,
               const vector<int>& csc_row_Index, const vector<data_t>& cscVal,
