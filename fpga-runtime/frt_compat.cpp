...
void Instance::SetBufferArgRaw(int index, internal::Tag tag,
  ...
  if (impl_->handle != nullptr) {
    CheckFfi(frt_instance_set_buffer_arg(
                 impl_->handle, static_cast<uint32_t>(index),
                 reinterpret_cast<uint8_t*>(arg.Get()), arg.SizeInBytes()),
             "frt_instance_set_buffer_arg");
  }
}
...
size_t Instance::SuspendBuf(int index) {
  return impl_->buffer_args.erase(index);
}
...
void Instance::Exec() {
  auto tic = std::chrono::steady_clock::now();
  if (impl_->handle != nullptr && !FLAGS_xosim_resume_from_post_sim &&
      !FLAGS_xosim_setup_only) {
